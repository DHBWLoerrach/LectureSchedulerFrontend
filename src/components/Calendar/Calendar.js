...
function getCurrentISOWeekAndYear() {
  ...
  const firstDayIndex =
    firstThursday.getDay() === 0 ? 7 : firstThursday.getDay();
...
const TempKalender = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const matchedModuleIds = await modules
          .filter(
            (subArray) =>
              Array.isArray(subArray[3]) && subArray[3].includes(user[5])
          )
  ...
  const handleCourseChange = (selectedOption) => {
    ...
    if (Array.isArray(sgModuleIds)) {
      ...
      setState((prev) => ({
        ...prev,
        moduleOptions,
        earliest: parseInt(foundSG[8]),
        latest: parseInt(foundSG[9]),
      }));
    } else {
      setState((prev) => ({
        ...prev,
  ...
  const handleSaveCalendar = async () => {
    ...
    try {
      ...
      const result = await UpdateCalendarAPI(
  ...
  return (
    <div style={{ padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <p className="text-center">
        Klicke auf ein Zeitfenster, um das ausgewählte Modul zuzuweisen.
      </p>
      <table
        style={{
          width: '100%',
