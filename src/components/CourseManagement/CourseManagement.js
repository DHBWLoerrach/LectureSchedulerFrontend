...
const CourseManagement = () => {
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      const apiResponse = await DeleteCourseAPI(selectedIds);
  ...
  const handleaddDepartment = (newDepartment) => {
    ...
    const projectArray = [
      newDepartment._id,
      newDepartment.Kursname,
      newDepartment.Studiengang,
      newDepartment.Semester,
    ];
    ...
    setState(prevState => ({
      ...
      allPersonnel: prevState.allPersonnel.map((department) => {
        if (department[0] === configuredDepartment._id) {
          return [
            department[0],
            configuredDepartment.Kursname,
            configuredDepartment.Studiengang,
            configuredDepartment.Semester,
          ];
        }
        return department;
      }),
  ...
  return (
    <div>
        ...
        <div className="col-sm-6 col-x1-3">
            <div className="bg-light rounded d-flex align-items-center justify-content-between p-4">
              ...
              <div className="ms-3">
                ...
                <h6 className="mb-0 text-end">{state.allPersonnel.length}</h6>
              </div>
            </div>
          </div>
      ...
      {state.shownewDepartmentWidget && <NewModuleWidget addDepartment={handleaddDepartment} />} {/* Conditionally render NewModuleWidget */}
      ...
      {state.showEditProjectWidget && <EditModuleWidget employee={state.configureThisDepartment} editDepartment={handleEditDepartment} />} {/* Conditionally render EditDepartmentWidget */}
    </div>
  );
};
